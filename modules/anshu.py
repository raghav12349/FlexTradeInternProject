...
def validate_dates(start_date, end_date):
    ...
    if (end - start).days < 365:
        return False, "Date range must be at least 1 year to produce a meaningful dividend analysis."
...
def rate_ticker(ticker: str, start_date: str, end_date: str) -> dict:
    ...
    if not divs:
        result["reasoning"].append("No dividend history found — growth stock, no income signal")
    ...
    current_year_count = sum(1 for d in divs if str(d.ex_dividend_date)[:4] == end_year)
    yearly_projected = dict(yearly)
    if end_year in yearly and current_year_count < normal_frequency:
        yearly_projected[end_year] = (yearly[end_year] / current_year_count) * normal_frequency
    ...
    if end_year in yearly:
        score += 2
        result["reasoning"].append(f"Paying dividend in {end_year} (+2)")
    else:
        score -= 2
        result["reasoning"].append(f"No dividend recorded in {end_year} (-2)")

    if end_year in yearly_projected and prev_year in yearly:
        change = yearly_projected[end_year] - yearly[prev_year]
        pct    = (change / yearly[prev_year]) * 100
        if change > 0:
            score += 3
            result["reasoning"].append(f"Dividend increased ~{pct:.1f}% vs previous year (+3)")
        elif change < 0:
            score -= 5
            result["reasoning"].append(f"Dividend cut ~{abs(pct):.1f}% vs previous year (-5)")
        else:
            result["reasoning"].append("Dividend flat vs previous year (0)")
    ...
    if best_streak >= 5:
        score += 8
        result["reasoning"].append(f"{best_streak} consecutive years of dividend growth (+8)")
    elif best_streak >= 3:
        score += 5
        result["reasoning"].append(f"{best_streak} consecutive years of dividend growth (+5)")
    elif best_streak >= 1:
        score += 3
        result["reasoning"].append(f"{best_streak} year(s) of consecutive growth (+3)")
    else:
        result["reasoning"].append("No consecutive growth streak (0)")
    ...
    if len(complete_years) >= 2:
        ...
        if missing:
            score -= 4
            result["reasoning"].append(f"Dividend suspended in {', '.join(sorted(missing))} (-4)")

    latest_full = complete_amounts[-1] if complete_amounts else 0
    if latest_full > 5:
        score += 2
        result["reasoning"].append(f"High annual payout ${latest_full:.2f}/share (+2)")
    elif latest_full > 2:
        score += 1
        result["reasoning"].append(f"Moderate annual payout ${latest_full:.2f}/share (+1)")
    ...
    if len(complete_amounts) >= 4:
        ...
        if recent_growth > 0 and early_growth > 0:
            if recent_growth > early_growth:
                score += 2
                result["reasoning"].append(f"Dividend growth accelerating ({recent_growth:.1f}% recent vs {early_growth:.1f}% earlier) (+2)")
            elif recent_growth < early_growth * 0.5:
                score -= 2
                result["reasoning"].append(f"Dividend growth slowing ({recent_growth:.1f}% recent vs {early_growth:.1f}% earlier) (-2)")
            else:
                result["reasoning"].append(f"Dividend growth steady ({recent_growth:.1f}% recent vs {early_growth:.1f}% earlier) (0)")
    ...
    if ratios:
        ...
        if market_cap:
            ...
            if market_cap >= 10_000_000_000:
                cap_label   = "Large Cap"
                cap_adjust  = 0
                cap_note    = "large cap — dividend commitments highly credible (0)"
            elif market_cap >= 2_000_000_000:
                cap_label   = "Mid Cap"
                cap_adjust  = 1
                cap_note    = "mid cap — bonus for paying dividends at this size (+1)"
            else:
                cap_label   = "Small Cap"
                cap_adjust  = -2
                cap_note    = "small cap — higher risk of dividend cuts (-2)"
            score += cap_adjust
            result["reasoning"].append(f"{cap_label} (${market_cap/1e9:.1f}B) — {cap_note}")
        ...
        if eps and eps > 0 and price and dividend_yield:
            ...
            if payout_ratio < 35:
                score += 3
                result["reasoning"].append(f"Payout ratio {payout_ratio:.1f}% — very sustainable (+3)")
            elif payout_ratio < 60:
                score += 1
                result["reasoning"].append(f"Payout ratio {payout_ratio:.1f}% — healthy (+1)")
            elif payout_ratio < 80:
                score -= 1
                result["reasoning"].append(f"Payout ratio {payout_ratio:.1f}% — elevated (-1)")
            else:
                score -= 4
                result["reasoning"].append(f"Payout ratio {payout_ratio:.1f}% — dangerously high (-4)")
        else:
